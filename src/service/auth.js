...
export const authLogin = async (req) => {
  try {
    ...
    const user =
      data.filter((v) => v.email === email && v.password === password) || []; //cek bila ada data yang sama
    if (user.length > 0)
      return { code: 200, status: "success", data, msg: "login sukses" };
    return {
      code: 404,
      status: "not found",
      data: null,
...
export const authRegister = async (req) => {
  try {
    ...
    if (user.length > 0) {
      ...
      await axios.post(`http://localhost:3002/data`, {
        id,
        email,
        password,
        username,
